...
""",
    ...
    p_read = sub.add_parser(
        "read", parents=[shared], help="Read a session (human-readable)"
    )
    p_read.add_argument("session", help="Session ID or partial match")
    p_read.add_argument(
        "-v", "--verbose", action="store_true", help="Include tool inputs and results"
    )
    p_read.add_argument(
        "-s",
        "--summary",
    ...
    p_queries.add_argument(
        "--session",
        default="",
        help="Session ID to extract from (omit for cross-session)",
    )
    p_queries.add_argument(
        "--keyword", "-k", default="", help="Filter queries containing keyword"
    )

    sub.add_parser(
        "corrections", parents=[shared], help="Find user correction patterns"
    )
    sub.add_parser(
        "decisions", parents=[shared], help="Find decision points in conversations"
    )
    sub.add_parser(
        "errors", parents=[shared], help="Extract error patterns from tool results"
    )
    ...
    sub.add_parser(
        "evolve-signals", parents=[shared], help="Generate signals JSON for Evolve page"
    )
    sub.add_parser(
        "evolve-patterns",
        parents=[shared],
        help="Generate patterns JSON for Evolve page",
    )

    p_ew = sub.add_parser(
        "evolve-write",
        help="Write/merge/delete Evolve tab data (validated, writes to SQLite)",
    )
    p_ew.add_argument(
        "--tab",
        required=True,
        choices=["profile", "memory", "rules", "signals", "patterns"],
        help="Target tab",
    )
    p_ew.add_argument(
        "--mode",
    ...
    p_ew.add_argument(
        "--ids", default="", help="Comma-separated ids/names for delete mode"
    )
    p_ew.add_argument(
    ...
    sub.add_parser(
        "profile-digest",
        parents=[shared],
        help="Pre-computed profile digest for sub-agents (JSON)",
    )

    sub.add_parser(
        "twin-stats", parents=[shared], help="Show Cognitive Handbook statistics"
    )
    ...
    p_tc.add_argument(
        "--status", default="", help="Filter by status (hypothesis/emerging/confirmed)"
    )
    ...
    sub.add_parser(
        "twin-write",
        help="Write/update/delete cognitive handbook entries from JSON stdin",
    )
    p_tcomp = sub.add_parser(
        "twin-compile", help="Compile Runtime Pack from cards + traits"
    )
    p_tcomp.add_argument(
        "--run-id",
        default="",
        help="Compile only artifacts from this Twin analysis run",
    )
    p_tcomp.add_argument(
        "--lang", default="zh", help="Output language: zh or en (default: zh)"
    )
    sub.add_parser(
        "twin-candidates", help="Validate candidate Twin operations without writing"
    )
    ...
    p_ts = sub.add_parser(
        "twin-search", parents=[shared], help="Search events/cards/traits by keyword"
    )
    p_ts.add_argument("resource", choices=["events", "cards", "traits"])
    p_ts.add_argument("--q", required=True, help="Search keyword")

    p_ta = sub.add_parser(
        "twin-add", help="Add a new event/card/trait (JSON from stdin)"
    )
    p_ta.add_argument("resource", choices=["events", "cards", "traits"])

    p_ted = sub.add_parser(
        "twin-edit", help="Edit an existing event/card/trait (JSON from stdin)"
    )
    ...
    p_tl.add_argument(
        "--run-id", default="", help="Require both endpoints to match this run id"
    )
    ...
    if save_path:
        ...
        print(
            f"Use: cat {save_path}  or  sed -n '1,100p' {save_path}  to read in segments"
        )
    else:
        cmds[args.command](args)
