...
def _get_analytics(handler) -> dict:
    ...
    heatmap = {
        ...
        "tools": sorted_tools,
        "data": {
            day: {t: tool_daily.get(day, {}).get(t, 0) for t in sorted_tools}
        ...
        with open(filepath, "r", encoding="utf-8", errors="replace") as f:
            for line in f:
                ...
                if source == "claude":
                    ...
                    elif rec_type == "response_item" and payload.get("type") in (
                        "function_call",
                        "custom_tool_call",
                    ):
                        ...
                        if isinstance(content, list):
                            for block in content:
                                if (
                                    ...
                                    if (
                                        fp
                                        and os.path.basename(fp) == basename
...
def _delete_session(session_id: str) -> dict:
    """Delete a session and all associated data from the database and index."""
    from chatview import db as _db
    from chatview import index as _idx

    # Validate session exists
    with _idx._index_lock:
        session = _idx._index.get("sessions", {}).pop(session_id, None)
    if not session:
        return {"ok": False, "error": "Session not found"}

    # Reset cache since index changed
    with _idx._cache_lock:
        _idx._result_cache.clear()
    ...
    try:
        ...
        # Delete evidence events referencing this session
        conn.execute("DELETE FROM evidence_events WHERE session_id=?", (session_id,))
    ...
    with _idx._index_lock:
        pname = session.get("projectName", "unknown")
        proj = _idx._index.get("projects", {}).get(pname)
        if proj:
            proj["sessionCount"] = max(0, proj["sessionCount"] - 1)
            if proj["sessionCount"] == 0:
                del _idx._index["projects"][pname]

    # Bump generation to trigger UI refresh
    _idx._index_gen += 1
    ...
    for sid, meta in sessions.items():
        ...
        if date_str:
            try:
                ...
                p["sessions_by_week"][week_key] = (
                    p["sessions_by_week"].get(week_key, 0) + 1
                )
            except Exception:
                pass
